...
class AvatarWidget extends StatelessWidget {
  const this({
    ...
    final double iconPadding = Dimensions.marginSmall,
    super.key,
  });
...
class PostDeleteUseCase extends UseCase<void, String> {
  ...
  this({
    /// Post Repo
    required final PostRepo postRepo,
  });
...
class Writer {
  ...
  this({
    ...
    required final List<Binding> lookUpBindings,

    /// Holds bindings, which lookup symbols through `FfiNative`.
    required final List<Binding> ffiNativeBindings,

    /// Holds bindings which don't lookup symbols.
    required final List<Binding> noLookUpBindings,

    required String className,

    /// The default asset id to use for [ffiNativeBindings].
    required final String? nativeAssetId,

    List<LibraryImport>? additionalImports,
    final String? classDocComment,
    final String? header,

    required final bool generateForPackageObjectiveC,

    required final bool silenceEnumWarning,

    required final List<String> nativeEntryPoints,
  }) {
    // ...
  }
...
class Resolver {
  ...
  // Note that we make the *private* constructor the declaring one here since
  // that lets us declare three fields based on parameters versus only one if
  // we had made the unnamed constructor the private one.
  this._({
...
class AssetTypeFilter extends EntityFilter {
  this({required var String assetType, var String? assetNameFilter});
...
class _FakePlatformViewsServiceProxy_11 extends _i1.SmartFake
  ...
  // This isn't declaring anything, but "this" is still shorter than the class
  // name.
  this(super.parent, super.parentInvocation);
}

// copy_pasta-0.0.7/lib/allyouneed/inventory_firestore/data.dart:1
class Item {
  // Constructor with initialization using named parameters
  this(var String name, var int quantity, var double price);
}
...
class ContentDetails {
  this({
    ...
    required final bool licensedContent,
...
class MySequence extends Sequence {
  ...
  // Here we take advantage of a non-field parameter and being able to
  // explicitly call the superclass constructor.
  this(final int numRepeat, {String name = 'mySequence'}) : super(name);
...
class NSNumberFormatter extends NSFormatter {
  // May as well use "this" because it's shorter.
  this._(
...
class ArtistDetailsScreen extends StatelessWidget {
  const this({super.key, required final Artist artist});
...
class GeneratorStatus implements MavlinkMessage {
  ...
  this({
    ...
    ///
    /// status
    required final MavGeneratorStatusFlag status,
    ...
    ///
    /// battery_current
    required final float batteryCurrent,
    ...
    ///
    /// load_current
    required final float loadCurrent,
    ...
    ///
    /// power_generated
    required final float powerGenerated,
    ...
    ///
    /// bus_voltage
    required final float busVoltage,
    ...
    required final float batCurrentSetpoint,
    ...
    ///
    /// runtime
    required final uint32_t runtime,
    ...
    required final int32_t timeUntilMaintenance,
    ...
    ///
    /// generator_speed
    required final uint16_t generatorSpeed,
    ...
    required final int16_t rectifierTemperature,
    ...
    required final int16_t generatorTemperature,
  });
...
class _FamilyOverrideRef {
  this(
    var FamilyOverride<Object?> override,
...
class AnimatedItemWidget extends StatelessWidget {
  ...
  // Here, supporting non-field parameters and initializer lists helps. If we
  // had non-const default values, this could use also field parameters for the
  // four configuration fields.
  this({
    super.key,
    required final Widget child,
    final ListAnimationType listAnimationType = ListAnimationType.Slide,
...
class SequentialInterceptor extends Interceptor {
  this({required final List<Interceptor> interceptors});
...
class DropdownItemPicker extends StatefulWidget {
  this({
    super.key,
    required final Widget pickerTitle,
    required final List<Widget> items,
    final ValueChanged<(Widget, int)>? onChanged,
    final int initialItemIndex = 0,
    final double scrollWheelHeight = 100,
...
class MenuActionAttributes {
  const this({final bool disabled = false, final bool destructive = false});
}
...
class EncodeGifCmd extends Command {
  this(
    super.input, {
    var int samplingFactor = 10,
    var DitherKernel dither = DitherKernel.floydSteinberg,
    var bool ditherSerpentine = false,
  });
...
class HandshakeResponseMessage {
  this(final String? error, final int? minorVersion);
...
class MatchVerticalState extends Equatable {
  this({
    required final GameFinalModel gameData,
    required final int countQuestions,
    required final List<GameLettersGameFinalModel> answers,
    final List<GlobalKey> answersKey = const [],
    final List<GlobalKey> questionsKey = const [],
    final List<GameLettersGameFinalModel> correctAnswers = const [],
    final bool isArabic = true,
    final bool isLetterVisible = false,
    required final List<GameImagesGameFinalModel> imageAnswers,
  });
...
class DSAppLovinAdWidget extends StatefulWidget {
  ...
  const this({super.key, required final DSAppLovinNativeAd ad});
...
class RositaImage extends LeafRenderObjectWidget {
  // Here, it's a toss-up as to which constructor should be declaring.
  const this.asset(
    String name, {
    super.key,
...
class InvalidKeyLength implements Exception {
  this(var int actual, var int expected);
...
class DBSQLMemoryAdapterContext
  ...
  this(final int id, final DBSQLMemoryAdapter sqlAdapter)
...
class RestartAppWidget extends StatefulWidget {
  this({super.key, required final Widget child});
